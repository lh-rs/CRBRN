...
'''
            ...
            loss = torch.sum(
                torch.mul(diff, 1 - pux) + torch.mul(
                    torch.where(diff < m, (m - diff), torch.zeros(diff.shape).cuda()), pux)) / N

            ContrastLoss
            loss = torch.sum((1 - pux) * torch.pow(diff, 2) +
                                         pux * torch.pow(torch.clamp(m - diff, min=0.0),2))/ N

            SCCNLoss: pux and diff are itertive                       
            loss = torch.sum(torch.mul(diff, 1 - pux)) / N
            pux = torch.where(diff < 0.1, torch.ones(pux.shape).cuda(), torch.zeros(pux.shape).cuda())
            '''
